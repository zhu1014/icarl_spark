...
class ICarl(IncrementalLearner):
    ...
    def __init__(self, args):
        ...
        self._update_every_time = args["update_every_time"]
        self._use_gcn = args["use_gcn"]
        self._task_epoch = args["task_epoch"]
        self._task_lr = args["task_lr"]
    ...
    def _before_task(self, train_loader, val_loader, task_id):
    ...
    def _train_new_class(self, train_loader, val_loader, task_id, task_info):
        logger.info("Train on new data again")
        train_net = self._network
        grad, act = None, None
        for epoch in range(2):
            ...
            for feature, target in zip(features, targets):
                if target in range(task_info["min_class"], task_info["max_class"] + 1):
                    features_new.append(feature)
                    targets_new.append(target)
            _class_mean_ = self._class_means.tolist()
            if self._connect_all:
                ...
                nbrs = NearestNeighbors(n_neighbors=self._connect_num, algorithm='auto').fit(
                    np.array(_class_mean_))
            # n_graph = nbrs.kneighbors_graph(_class_mean_).toarray()
            _class_mean_tensor = torch.tensor(_class_mean_, dtype=torch.float32)
            ...
            for i in range(len(_class_mean_)):
                ...
                generate_feature[i] = np.dot(np.array(dis2percent(_distances[i])).T, np.array(_class_mean_))
            ...
            for i, input_dict in enumerate(prog_bar, start=1):
                ...
                for input, target in zip(inputs, targets):
                    ...
                    if target in range(task_info["min_class"], task_info["max_class"] + 1):
                        ...
                        self._print_metrics(prog_bar, 0, 2, i)

        print("new class train again done")
        if len(self._multiple_devices) == 1 and hasattr(train_net.convnet, "record_mode"):
            train_net.convnet.normal_mode()



    def _train_task(self, train_loader, val_loader, task_id, inc_dataset, task_info):
        logger.debug("nb {}.".format(len(train_loader.dataset)))
        self._training_step(train_loader, val_loader, 0, self._n_epochs, task_id, inc_dataset, task_info)

    def _training_step(
        self, train_loader, val_loader, initial_epoch, nb_epochs, task_id, inc_dataset, task_info, record_bn=True, clipper=None
    ):
        ...
        print(self._network.convnet.out_dim)

        if len(self._task_epoch) > 1:
            print(self._task_epoch[task_id])
            nb_epochs = self._task_epoch[task_id]
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            features, targets_ = utils.extract_features(train_net, train_loader)
            generate_feature = self.cal_generate_feature(features, targets_, task_id, training_gcn)
            ...
            for i, input_dict in enumerate(prog_bar, start=1):
                ...
                if self._update_every_time:
                    generate_feature = self.cal_generate_feature(features, targets_, task_id)
    ...
    def _print_metrics1(self, prog_bar, epoch, nb_epochs, nb_batches):
        pretty_metrics = ", ".join(
            "{}: {}".format(metric_name, round(metric_value / nb_batches, 3))
            for metric_name, metric_value in self._metrics.items()
        )
    ...
    def cal_generate_feature(self, features, targets, task_id, training_gcn):
        # features, targets = utils.extract_features(train_net, train_loader)
        ...
        _class_mean_ = _class_mean_.tolist()
        ...
        if self._use_gcn:
            training_gcn = training_gcn.to(self._device)
            _class_mean_gcn = _class_mean_tensor.to(self._device)
            n_graph = torch.tensor(n_graph, dtype=torch.float32).to(self._device)
            gcn_outputs = training_gcn(_class_mean_gcn, n_graph, with_softmax=False)
            gcn_outputs = F.softmax(gcn_outputs, dim=1)

            gcn_outputs_norm = Normalize(gcn_outputs.cpu().detach().numpy())
        ...
        for i in range(len(_class_mean_)):
            ...
            if self._use_gcn:
                generate_feature[i] = np.dot(np.array(gcn_outputs_norm[i]).T, np.array(class_mean_))
            else:
                generate_feature[i] = np.dot(np.array(dis2percent(_distances[i])).T, np.array(class_mean_))
        #print(generate_feature[1].shape)
...
def Normalize(seq):
    ...
    for i in range(len(seq)):
        normalized = [(seq[i][j] / np.sum(seq[i])) for j in range(len(seq[i]))]
        seq[i] = normalized

    # 归一化(divide standard deviation)
        #normalized[i] = centerized / np.std(centerized, axis=0)
