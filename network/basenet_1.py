...
class BasicNet(nn.Module):
    ...
    def __init__(
        ...
        if classifier_kwargs["type"] == "fc":
            if not self.inc_flag:
                self.classifier = Classifier((self.convnet.out_dim) * 2, device=device, **classifier_kwargs)
                #print("additional_features", self.additional_features.shape)
            ...
            if not self.inc_flag:
                ...
                self.classifier = CosineClassifier(
                    self.convnet.out_dim *2, device=device, **classifier_kwargs
                )
        ...
        if rotations_predictor:
            ...
            self.rotations_predictor = nn.Linear(self.convnet.out_dim*2, 4)
        else:
    ...
    def forward(
        self, x, rotation=False, index=None, features_processing=None
    ):
        if hasattr(self,
        ...
        if rotation:
            ...
            clf_outputs = self.classifier(torch.cat((selected_features , self.additional_features), dim=1))
            #print(self.additional_features)
            #if not self.inc_flag:
                #self.classifier = Classifier(self.convnet.out_dim, device=self._device, **self._classifier_kwargs)
           ...
           # else:
            #    self.classifier = CosineClassifier(
            #        self.convnet.out_dim * 2, device=self._device, **self._classifier_kwargs
             #   )
             #   self.classifier.add_classes(n_classes)
            #    clf_outputs = self.classifier(
            #        torch.cat((selected_features , self.additional_features), dim=1)
            #    )
