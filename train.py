...
from lib import logger as logger_lib
from lib import metrics, results_utils, utils
import warnings
...
def _train(args, start_date, class_order, run_id):
    ...
    for task_id in range(inc_dataset.n_tasks):
        ...
        # ---------------
        model.eval()
        model.before_task(train_loader, val_loader if val_loader else test_loader, task_id)

        # -------------
        # 2. Train Task
        # -------------
        _train_task(args, model, train_loader, val_loader, test_loader, run_id, task_info, task_id, inc_dataset)

        # ----------------
        # 3. Conclude Task
        # ----------------
        model.eval()
        _after_task(args, model, inc_dataset, run_id, task_id, results_folder, train_loader, val_loader, test_loader, task_info)
...
def _train_task(config, model, train_loader, val_loader, test_loader, run_id, task_info, task_id, inc_dataset):
    if config["resume"] is not None and os.path.isdir(config["resume"]) \
       ...
       # class_means = pickle.load(f)[-1]
        ...
        model._train_task(train_loader, val_loader if val_loader else test_loader, task_id, inc_dataset, task_info)


def _after_task(config, model, inc_dataset, run_id, task_id, results_folder, train_loader, val_loader, test_loader,
                task_info):
    ...
    #model.train()
    #model._train_new_class(train_loader, val_loader if val_loader else test_loader, task_id, task_info)
